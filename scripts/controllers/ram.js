...
// -FIN----------------------Oculto botones segun corresponda---------------------------------------FIN
        ...
        vm.btnParticular = function () {
            ...
            angular.forEach(vm.atencion.datosPrestacionesInsert,function(v,k){

                var prestacionInsert = {
                    "urn:idPrestacion" : v["urn:idPrestacion"],
                    "urn:idConvenioPrestacion" : v["urn:idConvenioPrestacion"],
                    "urn:idEspecialidad" : v["urn:idEspecialidad"],
                    "urn:idMedico" : v["urn:idMedico"],
                    "urn:item" : v["urn:item"],
                    "urn:ges" :  v["urn:ges"],
                    "urn:rh" : v["urn:rh"],
                    "urn:cantidad" : v["urn:cantidad"],
                    "urn:monto" : v["urn:monto"],
                    "urn:bonificacion" : v["urn:bonificacion"],
                    "urn:copago" : "1",
                    "urn:medicoDerivado" :  v["urn:medicoDerivado"]

                  };
                  cargaDatos[k] = prestacionInsert;
            ...
            vm.atencion.datosPrestacionesInsert = [];

            vm.atencion.datosPrestacionesInsert = cargaDatos ;
...
console.log(vm.atencion);
            ...
            vm.atencion.datosBonificacionTotales[k] = 0;
                ...
                arrayLLenaParaInsert.push(
...

                                            {
                                                ...
                                                "urn:montoExcedente":vm.pago.montoPagoExcedenteOK,
                                                "urn:montoCCAF":"0",
 ...
 // validaciones --------------------------------------pago-------------------------------------------------------
    /*vm.validaCheque = function (valor){
                            ...
                            $("#contenido_mensaje").append("<div class='divide_mensajes'>Falta el n° de cheque</div>");
                            return false 
                              
                            }else if(isNaN($("#id_cheque_numero").val())){ // si tiene tecto

                               
                                $("#contenido_mensaje").append("<div class='divide_mensajes'>El n° de cheque solo debe contener números</div>");  
                                   return false
                            }
...
//----VALIDACIONES----
                switch ($("#for-pag").val()) { // switch para validar el tipo de pago con respectivos campos
                    ...
                    case 'EXCEDENTE':
                        {
                            vm.atencion.datosPago.totales.montoExcedenteAusar -= vm.pago.monto
                            break;
                        }
...
//Fin----VALIDACIONES----         
            ...
            angular.forEach(vm.agregaRowMediosPago, function (pago) { // recorro "vm.agregaRowMediosPago" para sumar los medios de pago 
                    ...
                    case 'EXCEDENTE':
                        {
                            ...
                            vm.datosPaciente.montoExcedente = parseInt(vm.datosPaciente.montoExcedente) - parseInt(montoPagoExcedente);  
              ...
              /*  console.log('vm.atencion.datosFolio');
                ...
                console.log("------------------------------------------FIn ----vm.agragarPago----------------------------------------------"); */     
                        ...
                        console.log(montoPagoExcedente);
        ...
        vm.removePago = function (row) {
            ...
            if(row.tipo == "EXCEDENTE"){
                ...
                vm.cambioAdevolver = 0;
                vm.datosPaciente.montoExcedente +=  row.monto;
                vm.atencion.datosPago.totales.montoExcedenteAusar +=row.monto;
