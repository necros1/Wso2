...
angular.module('medipassBE')
  .service('servicioMilliways', function ($stateParams,$state,servicioLocal,$timeout,$localStorage) {
    ...
    var url_valoriza = "http://bem.openpartner.cl/bono/bono_electronico_v2";
    var url = "http://bem.openpartner.cl/bono2/bono_electronico_v3";
      ...
      $.each(u,function(k,v){ 
       ...
       cargaDatosExcel = [
                     ...
                     v["ns:fechaHoraBono"],                         
                      ...
                      v["ns:copago"],  
/*
                      moment(v["ns:fechaHoraBono"]).unix(),
                      res[1],
                      v["ns:planGrupo"],


                      v["ns:usoExcedente"],
                      v["ns:rutAfiliado"]["ns:numero"]+"-"+v["ns:rutAfiliado"]["ns:dv"],
                      v["ns:nombreAfiliado"],
                      v["ns:apellidoAfiliado"],



                      v["ns:idEstatus"],
                      v["ns:idRazonSocial"],
                      v["ns:rutRazonsocial"]["ns:numero"]+"-"+v["ns:rutRazonsocial"]["ns:dv"],                  
                      v["ns:nombreFantasia"],
                      v["ns:nombreRazonsocial"],
...
//'id','sucursal','lugar de atencion','tipo','bono','atencion','fecha de emision','rut paciente','nombre paciente','rut medico tratante','medico tratante','isapre','bon isapre','compania de seguro','bon seguro','ccaf','bon ccaf','costo de atencion','monto','copago'
      vm.servicioLocal.consultaVentas.push(cargaDatos);

      console.log('------------------------vm.servicioLocal.consultaVentas------------------------------------');

      console.log(vm.servicioLocal.consultaVentasExcel);
...
/*    function  consultaMedicoEspecialidadPrestacion()
      {
        ...
        SOAPClient.invoke(url, "consultaMedicoEspecialidadPrestacion", pl, true,function(u){
          ...
          vm.servicioLocal.ArraycodigoEspecialiurl_valorizadad  = codigoEspecialidad;
    ...
    vm.horaGloval;
     ...
     // console.log(hora +":"+ minuto +":"+segundo);
      ...
      this.anulaBono = function (folioFinanciador){
        ...
        //    pl.add("urn:numeroConvenio",vm.form.prestacion.selected.numeroConvenio);
         ...
         /*   pl.add("urn:rutTratante","0084655500-5");*/
           ...
           //  pl.add("urn:rutBeneficiario",vm.datosPaciente.rut);        
            ...
            SOAPClient.invoke(url_valoriza, "valorizarPrestacion", pl, true,function (u)
                {
                    ...
                    //    $("#cantidad_lotes_modal").next().remove();
                        ...
                        $//("#alerta_mensajes").append("<button  class='btn gri-gri' type='submit' ng-click='vm.btnParticular()'>Atención Particular</button> ");
              
                        $("#Modal1").modal('show');
                        $("#close-modal").click(function(){
                        
                           /* vm.tipoAtenciones = 'PARTICULAR';
                            vm.atencion.datosAtencion.tipoAtencion =  vm.tipoAtenciones;*/
                            console.log( vm.tipoAtenciones);
                        });
...
//alert(u[0]["ns:fechaApertura"]);
    vm.servicioLocal.fechaAperturaCaja = u[0]["ns:fechaApertura"];


    vm.servicioLocal.montoAperturaCaja = u[0]["ns:montoApertura"];
    

    vm.servicioLocal.cantidadAtenciones = u[0]["ns:cantidadAtenciones"];
  ...
  this.detalleCajaMedioPago = function(tipoPago)
  {
    ...
    console.log(vm.servicioLocal.detalleTipoPagoSeleccionado);
  ...
  function detalleCajaMedioPago_callBack(u)
  {
	    ...
	    if(vm.servicioLocal.detalleTipoPagoSeleccionado == 1){
	    	...
	    	if(v["ns:montoEfectivo"] != 0){
			      ...
			      cargaDatos = { 
			                  ...
			                  /*  montoExcedente : v["ns:montoExcedente"],                    
			                    montoCheque : v["ns:montoCheque"],    */    
			                    copago : v["ns:copago"],
			                };
   ...
   vm.servicioLocal.datosDetalleCajaMedioPago = [];
    console.log(vm.servicioLocal.datosDetalleCajaMedioPago);
